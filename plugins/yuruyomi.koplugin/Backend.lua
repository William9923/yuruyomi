...
function Backend.createDownloadChapterJob(source_id, manga_id, chapter_id, chapter_num)
  return Backend.requestJson({
    ...
    method = 'POST',
    timeout = 15, -- Reduced timeout for job creation
    body = {
      source_id = source_id,
      manga_id = manga_id,
...
function Backend.createDownloadScanlatorChaptersJob(source_id, manga_id, scanlator, amount)
  ...
  return Backend.requestJson({
    ...
    method = 'POST',
    timeout = 15, -- Added timeout for consistency
    body = body
  })
end
...
--- @field message string Descriptive message about the operation result

--- Clears all reading history for a manga (marks all chapters as unread)
--- @param source_id string
--- @param manga_id string
--- @return SuccessfulResponse<OperationResult>|ErrorResponse
